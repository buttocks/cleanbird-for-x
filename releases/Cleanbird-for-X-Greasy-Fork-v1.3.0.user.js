...
(function () {
  ...
  function updatePremiumNags() {
    ...
    if (!primary) return;
    const markers = [...primary.querySelectorAll('span, strong')].filter(element => {
      const text = normalizedText(element);
      return text.includes("you aren't verified yet") || text.includes('you arent verified yet');
    });
    ...
    for (const marker of markers) {
      ...
      while (node && node !== primary) {
        ...
        const hasVerificationCopy = text.includes('get verified') && text.includes('stand out');
        const hasCloseControl = [...node.querySelectorAll('button, [role="button"]')].some(control => {
          const label = `${control.getAttribute('aria-label') || ''} ${control.getAttribute('title') || ''}`
            .trim()
            .toLowerCase();
          return label.includes('close') || normalizedText(control) === '×';
        });
        if (hasVerificationCopy && hasCloseControl) {
